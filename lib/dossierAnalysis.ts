/**
 * Tactical Dossier Analysis Engine
 * 
...
export interface ArmySpiritIcon {
  id: string;
  dossierHash: string;     // Hash of army list for uniqueness
...
export function analyzeDossier(armyList: ParsedArmyList): DossierAnalysis {
...
export interface DossierStrategicAnalysis {
 ...
 * @param context - The dossier context with army composition
...
export function buildDossierUserPrompt(
  ...
  const formatUnitCard = (u: DossierContext['units'][0]): string => {
    const lines: string[] = [];
...
`;
  ...
  const formatFactionContext = (factionCtx: DossierContext['factionContext']): string => {
