 ...
 *
 * Manages the credit-based system for brief generation.
 * - New users get 2 free generations
 * - Admin users bypass credit checks
 * - Credits are atomically deducted when generating a brief
 */
...
export async function checkAndDeductCredit(userId: string): Promise<CreditCheckResult> {
  try {
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { briefCredits: true, isAdmin: true }
    });
    ...
    const updatedUser = await prisma.user.update({
      ...
      select: { briefCredits: true }
    });

    return { 
      allowed: true, 
      remainingCredits: updatedUser.briefCredits,
      isAdmin: false 
    };
  } catch (error) {
...
/**
 * Check if a user can generate a brief (has credits) WITHOUT deducting.
...
export async function checkCreditsAvailable(userId: string): Promise<CreditCheckResult> {
  try {
    ...
    return {
      allowed: true,
      remainingCredits: user.briefCredits,
      isAdmin: false
    };
  } catch (error) {
...
export async function getUserCredits(userId: string): Promise<number> {
  try {
    ...
    return user.isAdmin ? Infinity : user.briefCredits;
  } catch (error) {
...
export async function adjustUserCredits(userId: string, amount: number): Promise<number> {
  try {
    const updatedUser = await prisma.user.update({
      where: { id: userId },
      data: { briefCredits: { increment: amount } },
      select: { briefCredits: true }
    });

    return updatedUser.briefCredits;
  } catch (error) {
