/**
 * Dossier Generator - Core Logic
 *
 * Extracts the core dossier generation functionality from the API route
...
import { checkAndDeductCredit } from '@/lib/dossierCredits';
import {
  DossierAnalysis,
...
const DOSSIER_MODEL = 'gemini-3-pro-preview';
...
export interface GenerateDossierOptions {
  ...
  userId: string;
  dossierId: string; // Required for background processing
...
/**
 * Generate a tactical dossier from a parsed army list.
 * This is the main entry point for direct function calls (no HTTP).
 * Updates the dossier record in the database with the results.
 */
export async function generateDossier(options: GenerateDossierOptions): Promise<GenerateDossierResult> {
  const { parsedArmy, localAnalysis, userId, dossierId, trace: externalTrace } = options;
  ...
  try {
    ...
    console.log(`🔄 Starting dossier generation for: ${dossierId}`);
    ...
    if (!trace) {
      trace = langfuse.trace({
        ...
        metadata: { faction, detachment, totalPoints: localAnalysis.totalPoints, dossierId, unitCount: localAnalysis.unitCount },
        tags: ['dossier-analysis', 'background-processing', `faction-${faction.toLowerCase().replace(/\s+/g, '-')}`]
      });
    } else {
      // Update external trace with dossier metadata
      trace.update({
        metadata: { faction, detachment, totalPoints: localAnalysis.totalPoints, dossierId, unitCount: localAnalysis.unitCount },
      });
    }
    ...
    const context = buildDossierContext(enrichedArmy, localAnalysis);
    enrichmentSpan.end({
      metadata: {
    ...
    const userPrompt = buildDossierUserPrompt(context, detachmentRulesPrompt, allFactionDatasheets);

    // Call Gemini for strategic analysis
    console.log(`🤖 Starting AI analysis for dossier: ${dossierId}`);

    const analysisGeneration = trace.generation({
      name: "gemini-dossier-analysis",
      model: DOSSIER_MODEL,
      input: [
    ...
    const stream = await gemini.models.generateContentStream({
      ...
      config: {
        ...
        responseJsonSchema: DOSSIER_RESPONSE_SCHEMA,
      }
    });
    ...
    const strategicAnalysis: DossierStrategicAnalysis = rawAnalysis;
    parseSpan.end({
      metadata: {
    ...
    const saveSpan = trace.span({
      name: "save-dossier-to-database",
      metadata: { dossierId, hasSpiritIcon: !!spiritIconUrl, suggestionsCount: listSuggestions.length }
    });
    await prisma.dossierGeneration.update({
      where: { id: dossierId },
      data: {
    ...
    saveSpan.end({ metadata: { savedDossierId: dossierId, initialVersionCreated: true } });

    console.log(`💾 Dossier completed: ${dossierId}`);
    ...
    console.error(`❌ Dossier generation error for ${dossierId}:`, error);
