...
export default function MyDossiersPage() {
  ...
  const fetchDossiers = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/dossier/list?${params}`);
      if (!response.ok) {
  ...
  const handleRename = async (id: string, newName: string) => {
    try {
      ...
      alert('Failed to rename dossier');
  ...
  const handleSelectAll = () => {
    if (data) {
      setSelectedIds(new Set(data.dossiers.map((d) => d.id)));
      setIsSelectionMode(true);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Delete ${selectedIds.size} dossier${selectedIds.size > 1 ? 's' : ''}? This cannot be undone.`)) return;

    setBulkActionLoading(true);
    try {
      const response = await fetch('/api/dossier/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      alert('Failed to delete dossiers');
    } finally {
  ...
  return (
    <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-grimlog-amber text-[10px] sm:text-xs font-mono tracking-wide hidden sm:block">
                        {data ? `${data.total} dossier${data.total !== 1 ? 's' : ''} in archive` : '++ LOADING ++'}
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                  {data.dossiers.map((dossier) => (
                    <DossierCard
                      ...
                      isSelected={selectedIds.has(dossier.id)}
      ...
      <BulkActionsToolbar
        ...
        totalCount={data?.dossiers.length || 0}
