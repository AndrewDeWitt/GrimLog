...
import { useDossierNotifications, PendingDossier } from '@/lib/dossier/DossierNotificationContext';
...
export default function DossierGalleryPage() {
  ...
  // Get pending dossiers from notification context
  const { pendingDossiers } = useDossierNotifications();
  ...
  const fetchDossiers = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/dossier/public?${params}`);
      if (!response.ok) {
    ...
    <>
      ...
      <main className="fixed inset-0 top-12 flex flex-col bg-grimlog-black">
        ...
        <div className="relative flex-1 overflow-auto min-h-0">
          ...
          {pendingDossiers.length > 0 && (
            <div className="mb-4">
              <h2 className="text-green-400 font-bold uppercase tracking-wider text-xs mb-2 flex items-center gap-2 font-mono">
                ...
                Processing ({pendingDossiers.length})
              </h2>
              <div className="space-y-2">
                {pendingDossiers.map((dossier: PendingDossier) => (
                  <div
                    key={dossier.id}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-green-400 font-bold text-sm truncate">
                        {dossier.faction || 'Analyzing...'}
                      </p>
                      <p className="text-green-600 text-xs font-mono">
                        {formatTimeAgo(dossier.startedAt)}
                      </p>
                    </div>
                  </div>
                ))}
          ...
          {/* Empty State */}
          {data && data.dossiers.length === 0 && !isLoading && (
            <div className="text-center py-16">
              <div className="inline-block p-6 bg-black/60 border border-green-500/30">
                <div className="text-green-400 font-mono text-sm mb-4">
                  {selectedFaction
                    ? `++ NO DOSSIERS FOUND FOR ${selectedFaction.toUpperCase()} ++`
                    : '++ ARCHIVE EMPTY ++'
                  }
                </div>
                <Link
          ...
          {/* Dossier Grid - Showcase/Portfolio Style */}
          {data && data.dossiers.length > 0 && !isLoading && (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                {data.dossiers.map((dossier) => {
                  const archetypeConfig = getArchetypeConfig(dossier.archetype);

                  return (
                    <Link
                      key={dossier.id}
                      href={dossier.shareToken ? `/dossier/share/${dossier.shareToken}` : `/dossier/${dossier.id}`}
                    ...
                    >
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-2.5">
                          ...
                          {archetypeConfig && (
                            <div className={`inline-flex items-center gap-1 text-[10px] sm:text-xs font-medium mb-1.5 ${archetypeConfig.color}`}>
                              ...
                              <span className="capitalize">{dossier.archetype?.replace('_', ' ')}</span>
                            </div>
                          )}
                      ...
                      <div className="p-2.5 bg-black/40 border-t border-white/5">
                        {dossier.tagline ? (
                          ...
                          <p className="text-grimlog-steel/60 text-xs mb-1">{dossier.faction}</p>
                        )}
