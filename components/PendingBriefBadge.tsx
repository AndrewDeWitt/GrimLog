 ...
 * PendingBriefBadge
 *
 * Shows a badge in the header for brief status:
 * - While generating: Shows "X Generating" (amber, spinning)
 * - When completed: Shows "Ready!" (green, checkmark) - clickable to go to brief
...
export default function PendingBriefBadge() {
  ...
  if (pendingCount > 0) {
    return (
      <button
        onClick={() => router.push('/brief/gallery')}
        ...
        title={`${pendingCount} brief${pendingCount > 1 ? 's' : ''} generating`}
      >
        {/* Spinning indicator */}
  ...
  if (recentlyCompleted.length > 0) {
    ...
    const handleClick = () => {
      // Clear the notification and navigate to the brief
      clearCompletedNotification(latestCompleted.id);
      router.push(`/brief/${latestCompleted.id}`);
    };
    ...
    return (
      <button
        ...
        title={`${latestCompleted.faction || 'Brief'} ready! Click to view.`}
      >
        {/* Checkmark */}
  ...
  if (recentlyFailed.length > 0) {
    ...
    const handleClick = () => {
      // Clear the notification and navigate to brief page to retry
    ...
    return (
      <button
        ...
        title={`Brief failed: ${latestFailed.errorMessage || 'Unknown error'}`}
      >
        {/* X icon */}
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
