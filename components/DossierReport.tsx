...
import { SynergyNetwork } from '@/components/dossier';
...
import {
  ...
  MatchupGuide,
} from '@/components/dossier/report';
...
interface DossierReportProps {
  ...
  onExport?: (exportData: DossierExportData) => void;
  dossierId?: string | null;
  dossierVisibility?: 'private' | 'link' | 'public';
  dossierShareToken?: string | null;
  listName?: string;
  // Edit mode props
  isEditMode?: boolean;
  onUpdateFunStats?: (funStats: DossierStrategicAnalysis['viralInsights']['funStats']) => void;
  ...
  onUpdateMatchups?: (matchups: DossierStrategicAnalysis['matchupConsiderations']) => void;
}
...
export default function DossierReport({
  ...
  const prepareSynergyExportData = (): DossierExportData['synergyData'] => {
  ...
  if (!analysis) {
    return (
      <div className="bg-grimlog-black p-8 text-center">
        <p className="text-gray-400">Loading dossier analysis...</p>
      </div>
    );
  }
    ...
    <div className="bg-grimlog-black pb-16">
      ...
      <div className="bg-grimlog-darkGray border-t-2 border-grimlog-steel p-4 text-center mt-4">
        <p className="text-gray-300 text-xs font-mono">
          Generated by Grimlog Tactical Dossier Engine • {new Date().toISOString()}
        </p>
