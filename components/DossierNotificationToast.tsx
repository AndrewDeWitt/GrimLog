 ...
 *
 * Shows toast notifications when dossiers complete or fail.
 * Clickable to navigate to the completed dossier.
 *
...
import { useDossierNotifications, CompletedDossier, FailedDossier } from '@/lib/dossier/DossierNotificationContext';

const TOAST_DURATION = 10000; // 10 seconds before auto-hide (not dismiss)

export default function DossierNotificationToast() {
  ...
  const [currentNotification, setCurrentNotification] = useState<{
    ...
    dossier: CompletedDossier | FailedDossier;
  } | null>(null);

  // Update current notification when new ones arrive
  useEffect(() => {
    if (latestFailed) {
      setCurrentNotification({ type: 'failed', dossier: latestFailed });
    } else if (latestCompleted) {
      setCurrentNotification({ type: 'completed', dossier: latestCompleted });
    } else {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      autoHiddenIdsRef.current.add(currentNotification.dossier.id);
      setCurrentNotification(null);
    }, TOAST_DURATION);
  ...
  const { type, dossier } = currentNotification;
  const isSuccess = type === 'completed';

  const handleClick = () => {
    if (isSuccess) {
      clearCompletedNotification(dossier.id);
  ...
  const handleClose = (e: React.MouseEvent) => {
    ...
    if (isSuccess) {
      ...
      clearFailedNotification(dossier.id);
    }
  };
  ...
  const title = isSuccess ? 'Dossier Ready!' : 'Dossier Failed';
  const message = isSuccess
    ? `Your ${dossier.faction || 'army'} tactical dossier is ready. Click to view.`
    : `Failed to generate ${dossier.faction || 'army'} dossier. ${(dossier as FailedDossier).errorMessage || 'Please try again.'}`;
