...
interface BriefVersionHistoryPanelProps {
  briefId: string;
...
/**
 * BriefVersionHistoryPanel - Slide-out panel showing version history
 */
export function BriefVersionHistoryPanel({
  briefId,
  currentVersion,
  isOpen,
  onClose,
  ...
  const fetchVersions = async () => {
    ...
    try {
      const response = await fetch(`/api/brief/${briefId}/versions`);
      if (!response.ok) {
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-grimlog-steel/70 text-sm mt-2">
                Use &quot;Save Version&quot; to create snapshots of your brief
              </p>
            </div>
          ) : (
...
export default BriefVersionHistoryPanel;
