...
export function DossierShareModal({
  isOpen,
  onClose,
  dossierId,
  ...
  if (!isOpen || !dossierId) return null;

  const shareUrl = shareToken 
    ? `${window.location.origin}/dossier/share/${shareToken}`
    : null;
  ...
  const handleShareTwitter = () => {
    if (!shareUrl) return;
    const text = `Check out my ${faction || ''} Tactical Dossier!`;
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-3 p-3 bg-grimlog-black border border-grimlog-steel rounded cursor-pointer hover:border-grimlog-amber transition-colors">
                ...
                <div>
                  ...
                  <p className="text-grimlog-steel text-xs mt-0.5">Only you can view this dossier</p>
                </div>
              </label>
