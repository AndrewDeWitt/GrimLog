...
export default function DossierCard({
  ...
  const [editName, setEditName] = useState(dossier.listName || '');
  const [isSaving, setIsSaving] = useState(false);

  const isPrivate = dossier.visibility === 'private';
  const archetypeConfig = dossier.archetype ? ARCHETYPE_CONFIG[dossier.archetype] : null;
  ...
  const handleSaveRename = async () => {
    ...
    if (trimmedName === (dossier.listName || '')) {
      setIsEditing(false);
      return;
    }
    setIsSaving(true);
    try {
      await onRename(dossier.id, trimmedName);
      setIsEditing(false);
    } catch {
